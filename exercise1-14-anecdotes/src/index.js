...
const App = (props) => {
    ...
    const initializeArray = () => {
        const templateVotes = new Array(anecdotes.length).fill(0);
        return templateVotes;
    }
    
    const [votes, setVote] = useState( () => initializeArray() );
    ...
    return (
        <div>
            ...
            <h2> The best piece of advice, relatively speaking </h2>
            <p> {getLeader()} </p>
            <p> Vote total of the current leader: {votes[anecdotes.indexOf(getLeader())]}</p>
        </div>
    )
}
